export function sortCategoriesByOrder(categories: FoodCategory[]): FoodCategory[] {
  return [...categories].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
}

export function getMenuItemOptionGroupIdSet(item: Pick<MenuItem, "optionGroupIds">): Set<string> {
  return new Set((item.optionGroupIds ?? []).map((ref) => String(ref.optionGroupId)));
}
...
export function getItemOptionGroupsInDisplayOrder(
  ...
  const entries: { group: OptionGroup; sortOrder: number; menuIndex: number }[] = [];
  (item.optionGroupIds ?? []).forEach((ref, menuIndex) => {
    const g = byId.get(String(ref.optionGroupId));
    if (g) entries.push({ group: g, sortOrder: ref.order, menuIndex });
  });
  entries.sort((a, b) => a.sortOrder !== b.sortOrder ? a.sortOrder - b.sortOrder : a.menuIndex - b.menuIndex);
...
export function appendOrderItemOptionsForGroup(
  ...
  for (const optionId of group.optionIds ?? []) {
    ...
    if (option) { target.push({ name: option.name, price: option.price, quantity }); seen.add(optionId); }
  }
  for (const [optionId, quantity] of Object.entries(selectedByOptionId)) {
    if (seen.has(optionId) || quantity <= 0) continue;
    const option = allOptions.find((o) => o.id === optionId);
    if (option) target.push({ name: option.name, price: option.price, quantity });
  }
}

export function getFirstCategoryItems(categories: FoodCategory[], menuItems: MenuItem[]): MenuItem[] {
  ...
  for (const rawId of sorted[0].itemIds ?? []) {
...
export function getVisibleMenuItemsInCategoryOrder(categories: FoodCategory[], menuItems: MenuItem[]): MenuItem[] {
  const itemById = new Map<string, MenuItem>();
  for (const item of menuItems) {
    if (item.id != null) itemById.set(String(item.id), item);
  }
